...
import { AssetWriteoffService, LoginService, AssetWriteoff, cDate, NgbDateFRParserFormatter } from '../../../../shared';
import { TextMaskModule } from 'angular2-text-mask';
import swal from 'sweetalert';
import { TemplateBinding } from '@angular/compiler';

@Component({
  selector: 'asset-writeoff',
  templateUrl: './asset-writeoff.component.html',
  styleUrls: ['./asset-writeoff.component.scss'],
...
priviledged_Offices: any;
  ...
  constructor(private service: AssetWriteoffService, private LoginService: LoginService, private modalService: NgbModal) { 
 ...
 setPageInfo(value) {
    ...
    this.service.getAssetDepreciation(ID)
      .subscribe(response => {
        ...
        if (response.json() != null) {
          ...
          this.depreciationamount = list[0].depvalue;
  ...
  saveData() {
    ...
    var data = new AssetWriteoff(0, this.itemcode, this.depreciationamount, this.bookvalue, this.writeoffdate.getDateFinal(), this.writeoffvalue, this.remarks,this.logedInUserID, this.UserSessionID);
    console.log(data);
      ...
      .subscribe(response => {
        ...
        this.service.getOffice(this.priviledged_Offices)
          .subscribe(response => {
            ...
            this.isLoading =true;
            this.service.getItem(this.officecode)
              .subscribe(response => {
                this.isLoading = false;
                if (response.json() != null) {
                  this.item = this.getDropdownList(response.json(), "itemcode", "itemname");
                  ...
                  this.writeoffvalue = list[0].disposalprice;
                  this.writeoffdate.setdate(list[0].writeoffdate)
