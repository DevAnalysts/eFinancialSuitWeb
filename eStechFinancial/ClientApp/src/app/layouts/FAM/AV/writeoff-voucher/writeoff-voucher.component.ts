...
import { WriteoffVoucherService, LoginService, WriteoffVoucher, AssetDepreciateDetails, cDate, NgbDateFRParserFormatter, voucher } from '../../../../shared';
import swal from 'sweetalert';
@Component({
    selector: 'writeoff-voucher',
    templateUrl: './writeoff-voucher.component.html',
    styleUrls: ['./writeoff-voucher.component.scss'],
    providers: [{ provide: NgbDateParserFormatter, useClass: NgbDateFRParserFormatter }]
})
export class WriteOffVoucherComponent implements OnInit {
 ...
 priviledged_Offices: any;
    constructor(private service: WriteoffVoucherService, private ngbDateParserFormatter: NgbDateParserFormatter, private LoginService: LoginService,
    ...
    saveData() {
        ...
        for (let i = 0; i < this.vouchers.length; i++) {
            if (this.vouchers[i].status == true) {
                emptyRow.writeOffID = this.vouchers[i].writeOffID;
                emptyRow.creationdate = this.voucherdate.getDateFinal();
                emptyRow.writeDate = this.vouchers[i].writeDate;
                emptyRow.writeOffValue = this.vouchers[i].writeOffValue;
                data.push(emptyRow);
            }
        }
