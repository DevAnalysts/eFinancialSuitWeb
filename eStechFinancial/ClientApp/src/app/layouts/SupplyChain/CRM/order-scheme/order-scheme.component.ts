...
@Component({
  selector: 'order-scheme',
  templateUrl: './order-scheme.component.html',
  styleUrls: ['./order-scheme.component.scss'],
  encapsulation: ViewEncapsulation.None,
  //host: { '(window:keydown)': 'hotkeys($event)' },
  providers: [{ provide: NgbDateParserFormatter, useClass: NgbDateFRParserFormatter }]
})
export class OrderSchemeComponent implements OnInit {
  ////////////////////////////////////////
  ...
  getRouteWiseScheme() {
    ...
    if (this.areacode != 0) {
      this.isLoading = true;
      this.service.getRouteWiseScheme(this.areacode, this.schemedate.getDateFinal())
        .subscribe(response => {
          this.isLoading = false;
          if (response.json() != null) {
    ...
    if (this.itemcode != 0) {
      if (this.amount > 0) {
        if (this.totalactualqty > 0) {
          ...
          swal('Total Actual Qty Should Be Greater Than Zero!')
      }
      else
        swal('Select Amount!')
    }
    else
      swal('Select Scheme Item!')
  }
  //updateData
  updateData() {
