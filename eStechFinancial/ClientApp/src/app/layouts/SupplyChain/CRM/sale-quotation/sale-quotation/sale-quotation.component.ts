...
import { LoginService, SearchFilterService, DayEndService, customer, CustomerService, Customers, cDate, NgbDateFRParserFormatter, FileAttachmentService, EmailAlertService, PermissionUtility, SMSMessage, SmsAlertService, sendWhatsappMessage, CommonUtility, SaleOrderService, saleOrder } from '../../../../../shared';
import swal from 'sweetalert';
import { Router } from '@angular/router';
import { Content } from '@angular/compiler/src/render3/r3_ast';
import { saleQuotationDetails } from '@shared/models/SupplyChain/CRM/saleQuotationDetails';
...
export class SaleQuotationComponent implements OnInit, AfterViewChecked {
  ...
  unit_Price: any = 0;
  payments: any[];
  saleQuotationDetails: any[];
  itemStockIMEI: any[];
  stocks: any[];
  saleQuotationDetail: any;
  selectedItem: Object = {};
  ...
  constructor(
    ...
    private service: SaleQuotationService,
    private qoutationOrderservice: SaleOrderService,
    ...
    this.saleQuotationDetails = new Array<saleQuotationDetails>();
  ...
  ngOnInit() {
    ...
    this.showDueDate();
    this.getCurrentDay();
    this.searchSaleQuotations('');
    this.getAreaEnableStatus();
    this.settingAllowPriceList();
    this.getTaxRate();
    this.PermissionUtility.setPagePermissions(140003);
  ...
  //getCurrentDay
  getCurrentDay() {
    this.qO_Date.setDate(this.LoginService.getSession('currentOpenDay'));
  ...
  getCompanyTemplate() {
    if (this.LoginService.getSession('ShowCompanyTemplate') != '1') {
      ...
      this.templateshow = '';
      this.isLoading = true;
      this.service.getCompanyTemplate(this.userCurrentOffice, 140003, this.IsSaleTaxInv)
        .subscribe(response => {
  ...
  //changeQODate
  changeQODate(pO_Date) {
    if (this.qO_Date.getStandardDate() < this.dayEndDate.getStandardDate())
      this.qO_Date.setDate(this.dayEndDate.getDateFinal());
  } 
  //changeDeliveryDate
  ...
  //searchOrderDetails
  searchSaleQuotations(value: string) {
    this.isLoading = true;
    this.service.searchSaleQuotations(value, this.userPrivilegedOffice)
      .subscribe(response => {
  ...
  getPriviledgedOffices() {
    this.qoutationOrderservice.getSaleOfficers(this.userPrivilegedOffice, this.saleOfficerDesignation)
      .subscribe(response => {
  ...
  getCustomers(AreaID) {
    // this.isLoading = true;
    this.service.getCustomersForQO(AreaID)
      .subscribe(response => {
  ...
  getUnit(mood)
  {
    if(mood == 0)
    {
      this.qoutationOrderservice.getUnits(mood).subscribe(response => {
...
*/
  ...
  changeCustomer(e: any) {
    ...
    this.service.getContacts(this.customer_ID)
      .subscribe(response => {
        ...
        this.service.getUnitPrice(this.item_Code, this.customerCategoryId, this.allowPriceList, this.userCurrentOffice, this.userCurrentWarehouse, this.customer_ID,0)
          .subscribe(response => {
            if (response.json() != null) {
              this.unitPrices = (response.json());
              this.item_Code = this.unitPrices[0].item_Code;
  ...
  TotalCost() {
    var total_Cost = 0;
    if (this.saleQuotationDetails.length > 0) {
      for (var count = 0; count < this.saleQuotationDetails.length; count++) {
        total_Cost += this.saleQuotationDetails[count].unit_Price * this.saleQuotationDetails[count].quantity;
      }
    }
  ...
  TotalDiscount() {
    ...
    if (this.saleQuotationDetails.length > 0) {
      for (var count = 0; count < this.saleQuotationDetails.length; count++) {
        total_Discount += ((this.saleQuotationDetails[count].unit_Price * this.saleQuotationDetails[count].quantity) * this.saleQuotationDetails[count].discount_Rate / 100);
      }
    }
  ...
  TotalTax() {
    ...
    if (this.saleQuotationDetails.length > 0) {
      for (var count = 0; count < this.saleQuotationDetails.length; count++) {
        total_Tax += ((this.saleQuotationDetails[count].unit_Price * this.saleQuotationDetails[count].quantity) - ((this.saleQuotationDetails[count].unit_Price * this.saleQuotationDetails[count].quantity) * this.saleQuotationDetails[count].discount_Rate / 100)) / 100 * this.saleQuotationDetails[count].tax_Rate;
      }
    }
  ...
  TotalAmount() {
    ...
    if (this.saleQuotationDetails != null) {
      if (this.saleQuotationDetails.length > 0) {
        for (var count = 0; count < this.saleQuotationDetails.length; count++) {
          total_Amount += (((this.saleQuotationDetails[count].unit_Price * this.saleQuotationDetails[count].quantity) - ((this.saleQuotationDetails[count].unit_Price * this.saleQuotationDetails[count].quantity) * this.saleQuotationDetails[count].discount_Rate / 100)) + ((this.saleQuotationDetails[count].unit_Price * this.saleQuotationDetails[count].quantity) - ((this.saleQuotationDetails[count].unit_Price * this.saleQuotationDetails[count].quantity) * this.saleQuotationDetails[count].discount_Rate / 100)) / 100 * this.saleQuotationDetails[count].tax_Rate);
        }
  ...
  changeDiscRateEdit(i:saleQuotationDetails){ 
    ...
    { 
      ...
      if(DiffPrice>0){
        ...
        this.discount_Amount = (this.Quantity.toFixed(2)) * (this.unit_Price.toFixed(2)) * (this.Discount_Rate.toFixed(2)) / 100
          ...
          this.disc_Price = this.unit_Price
        }
    } 
  }
  ...
  //changeQuantityEdit
  changeQuantityEdit(i: saleQuotationDetails, quantity) {
    if (quantity <= 0) {
      i.quantity = 1;
    }
  }
  ...
  //changeUnitPriceEdit
  changeUnitPriceEdit(i: saleQuotationDetails, unit_Price) {
    if (unit_Price <= 0) {
      i.unit_Price = 1;
    }
  }
  ...
  //updateItem 
  updateItem(i: saleQuotationDetails, e: any) {
  ...
  //changeMode
  changeMode(idx: any, i: saleQuotationDetails, Mode: any) {
    this.DelayCheck = true;
    ...
    if (this.saleQuotationDetails.length > 0) {
      for (var count = 0; count < this.saleQuotationDetails.length; count++) {
        if (this.saleQuotationDetails[count].item_Code == i.item_Code && idx != count) {
          flag = true;
          break;
        }
      }
      ...
      this.service.getStocks(i.item_Code, this.userCurrentOffice, this.userCurrentWarehouse)
        .subscribe(response => {
          ...
          if (Mode == 0) {
            ...
            console.log("Index : " + idx + "--- Page :" + this.g);
            this.saleQuotationDetails.splice(idx, 1);
            //this.getItems();
            this.Quantity=1;
            this.Discount_Rate=0;
  ...
  //addGrid
  addGrid(item_Code: any, item_Name: any, Unit_Price: any, Quantity: any, Discount_Rate: any, Tax_Rate: any,unit:any) {
        ...
        if (this.stock_Qty != null && stockQty >= Qty) {
          if (item_Name != null) {
            if (Quantity > 0) {
              ...
              if (this.saleQuotationDetails.length > 0) {
                for (var count = 0; count < this.saleQuotationDetails.length; count++) {
                  if (this.saleQuotationDetails[count].item_Code == item_Code) {
                    flag = true;
                    serialNo = count + 1;
                    break;
                  }
              ...
              if (flag == false) {
                if (this.hide == false) {
                  this.saleQuotationDetails.push(new saleQuotationDetails(0, 0, item_Code, item_Name, Unit_Price, Quantity, (Unit_Price * Quantity), Discount_Rate, ((Quantity) * (Unit_Price) * (Discount_Rate) / 100), this.taxratecode, 0, 0, 0, ((Quantity) * (Unit_Price) - ((Quantity) * (Unit_Price) * (Discount_Rate) / 100)).toFixed(2), 0, this.stock_Qty, this.pending,unit,unit,this.disc_Price,this.looseStock_Qty));
                  this.Quantity=1;
                  this.Discount_Rate=0; 
                  this.taxratecode = this.taxrateList[0].taxrateid;
                  this.Tax_Rate = this.taxrateList[0].taxrate;
                  ...
                  this.saleQuotationDetails.push(new saleQuotationDetails(0, 0, item_Code, item_Name, Unit_Price, Quantity, (Unit_Price * Quantity), Discount_Rate, ((Quantity * Unit_Price) * (Discount_Rate) / 100), this.taxratecode, Tax_Rate, ((((Quantity * Unit_Price) - ((Quantity * Unit_Price) * (Discount_Rate) / 100)) / 100) * Tax_Rate).toFixed(2), 0, (((Quantity * Unit_Price) - ((Quantity * Unit_Price) * (Discount_Rate) / 100)) + ((((Quantity * Unit_Price) - ((Quantity * Unit_Price) * (Discount_Rate) / 100)) / 100) * Tax_Rate)).toFixed(2), 0, this.stock_Qty, this.pending,unit,unit,this.disc_Price,this.looseStock_Qty));
                  this.Quantity=1;
                  this.Discount_Rate=0;
                  this.taxratecode = this.taxrateList[0].taxrateid;
                  this.Tax_Rate = this.taxrateList[0].taxrate;
                }
                //console.log('TAXAMOUNT', this.saleQuotationDetails);
  ...
  //IfExists
  IfExists(saleQuotation_ID) {
    this.service.IfExists(saleQuotation_ID)
      .subscribe(response => {
  ...
  onNavigate(pth) {
    ...
    if (pth == "/duplicate-rpt-rdlc") {
      ...
      sessionStorage.setItem('reportID', "19");
      window.open(pth, "_blank");
    }
  ...
  //saveOrder
  saveOrder(saleQuotation_ID: any, qO_Date: any, office_Code: any, order_Envoy: any, customer_ID: any, contact_ID: any,
    delivery_Date: any, method_Id: any, payment_ID: any, frieght_Id: any, total_Cost: any, total_Discount: any,
    total_Tax: any, freight_Chrgs: any, total_Amount: any, paid_Amount: any, requotation_ID: any, quotation_ID: any, quotation_Type: any, remarks: any,
    cancel: any, company_ID: any, manual_Discount: any, entry_Date: any, user_ID: any, is_Update: any, qOGUID: any) {
    $("#submitAdd").prop("disabled", true);
    $("#submitAddMore").prop("disabled", true);
    this.allowOfficeWarehouse();
    if (this.customer_ID > 0) {
      if (total_Amount > 0) {
        ...
        this.isLoading = true;
        var order = new saleQuotation(saleQuotation_ID, this.qO_Date.getDateFinal(), this.userCurrentOffice, order_Envoy, customer_ID, contact_ID,
          ...
          0, 1, manual_Discount, entry_Date, 1, false, this.guid, this.IsDirect, this.sD_Rate, this.sD_Amount, this.specialDiscount, this.exchange, 0, this.logedInUserID, this.UserSessionID, this.templatecode, this.refCustomerID, this.userCurrentWarehouse, this.saleQuotationDetails, this.itemStockIMEI);
        ...
        this.service.saveOrder(order).then(
          (response) => {
            ...
            //this.getDetailsByID(this.sale_Order_ID,'')
            if (this.isView == true) {
  ...
  //updateOrder
  updateOrder(saleQuotation_ID: any, qO_Date: any, office_Code: any, order_Envoy: any, customer_ID: any, contact_ID: any,
    delivery_Date: any, method_Id: any, payment_ID: any, frieght_Id: any, total_Cost: any, total_Discount: any,
    total_Tax: any, freight_Chrgs: any, total_Amount: any, paid_Amount: any, requotation_ID: any, quotation_ID: any, quotation_Type: any, remarks: any,
    cancel: any, company_ID: any, manual_Discount: any, entry_Date: any, user_ID: any, is_Update: any, qOGUID: any) {
    this.allowOfficeWarehouse();
    if (this.customer_ID > 0) {
      if (total_Amount > 0) {
        ...
        var order = new saleQuotation(saleQuotation_ID, this.qO_Date.getDateFinal(), this.userCurrentOffice, order_Envoy, customer_ID, contact_ID,
          ...
          1, 1, manual_Discount, entry_Date, 1, true, this.guid, this.IsDirect, this.sD_Rate, this.sD_Amount, this.specialDiscount, this.exchange, 0, this.logedInUserID, this.UserSessionID, this.templatecode, this.refCustomerID, this.userCurrentWarehouse, this.saleQuotationDetails, this.itemStockIMEI);
 ...
 //Convert To Order
 convertToOrder(saleQuotation_ID: any, qO_Date: any, office_Code: any, order_Envoy: any, customer_ID: any, contact_ID: any,
  delivery_Date: any, method_Id: any, payment_ID: any, frieght_Id: any, total_Cost: any, total_Discount: any,
  total_Tax: any, freight_Chrgs: any, total_Amount: any, paid_Amount: any, requotation_ID: any, quotation_ID: any, quotation_Type: any, remarks: any,
  cancel: any, company_ID: any, manual_Discount: any, entry_Date: any, user_ID: any, is_Update: any, qOGUID: any) {
  $("#submitAdd").prop("disabled", true);
  ...
  if (this.customer_ID > 0) {
    if (total_Amount > 0) {
      ...
      var order = new saleOrder(0, this.qO_Date.getDateFinal(), this.userCurrentOffice, order_Envoy,this.areacode, customer_ID, contact_ID,
        this.delivery_Date.getDateFinal(), method_Id, payment_ID, frieght_Id, total_Cost, total_Discount,
        total_Tax, freight_Chrgs, total_Amount, paid_Amount, 1, saleQuotation_ID, "New", remarks,
        0, 1, manual_Discount, entry_Date, 1, false, this.guid, this.IsDirect, this.sD_Rate, this.sD_Amount, this.specialDiscount, this.exchange, 0, this.logedInUserID, this.UserSessionID, this.templatecode, this.refCustomerID, this.userCurrentWarehouse, this.saleQuotationDetails, this.itemStockIMEI);

      this.qoutationOrderservice.saveOrder(order).then(
        (response) => {
  ...
  //getDetailsByID
  getDetailsByID(saleQuotation_ID, content) { 
    this.alerts = []
    this.alerts.push(
      {
        id: 4,
    ...
    sessionStorage.setItem('ID', saleQuotation_ID);
    this.mode = true;
    this.btnmode = false;
    //this.edit();
    this.IfExists(saleQuotation_ID);

    this.isLoading = true;
    this.service.getDetailsByID(saleQuotation_ID)
      .subscribe((o: saleQuotation) => {
        this.isLoading = false;
        this.attachments = [];
        console.clear();
        console.log('saleQuotation', o);
        this.saleQuotation_ID = o.saleQuotation_ID;
        sessionStorage.setItem('exchange', "-1");
        sessionStorage.setItem('ID', this.saleQuotation_ID.toString());
        sessionStorage.setItem('customerID', o.customer_ID.toString());
        this.qO_Date.setDate(o.qO_Date);
        ...
        this.saleQuotationDetails = o.saleQuotationDetails;
        ...
        for (let i = 0; i < this.saleQuotationDetails.length; i++) {

          this.isLoading = true;
          this.service.getUnitPrice(this.saleQuotationDetails[i].item_Code, this.customerCategoryId, this.allowPriceList, this.userCurrentOffice, this.userCurrentWarehouse, this.customer_ID,0)
            .subscribe(response => {
          
              this.isLoading = false;
              this.unitPrices = (response.json());
              if(this.unitPrices !==null){
              this.saleQuotationDetails[i].stock_Qty = this.unitPrices[0].stock_Qty;
              this.saleQuotationDetails[i].pending = this.unitPrices[0].pending;
              }
            });
        }
        if (this.sessionEnableTextboxCustomer != 1) {
          ...
          this.isLoading = true;
          this.service.getCustomersForQO(0)
            .subscribe(response => {
              this.isLoading = false;
              this.customers = this.getDropdownList(response.json(), "customer_ID", "customer_Name");
              this.isLoading = false;
  ...
  saveCustomer() {
    ...
    if (this.addCustomerName != "") {
      ...
      if (this.LoginService.getSession('MandatoryCellNo') == '1' && this.addCustomerCell == '') {
        ...
        var customer = new Customers(0, this.addCustomerName, this.categorycode, 0, this.addCustomerName, this.addCustomerPhone, this.addCustomerCell, this.addCustomerCell, this.addCustomerPhone, '', '', this.addCustomerEmail, '', 0, 1, 1, 1, 1, 1, 0, 1, this.guid, '', 0, 0, this.addCustomerAddress, '', '', this.citycode, 1, 1, this.addCustomerStatus, 1, this.logedInUserID, this.UserSessionID, 1, 1, this.refCustomerID, 0);
        ...
        this.customerService.saveCustomer(customer).then(
          (response) => {
            ...
            this.service.getCustomersForQO(this.areacode)
              .subscribe(response => {
                this.customers = this.getDropdownList(response.json(), "customer_ID", "customer_Name");
  ...
  openSendMail(content) {
    ...
    this.modalReferenceMail = this.modalService.open(content, { size: 'xl' });
  ...
  //changeTaxRate
  changeTaxRateGrid(j: saleQuotationDetails) {
  ...
  this.is_edit=true;
   ...
   { 
     for (let i = 0; i < this.saleQuotationDetails.length; i++) {
         ...
         this.saleQuotationDetails[i].taxrateid = this.taxratecode; 
         this.saleQuotationDetails[i].net_Amount-=this.saleQuotationDetails[i].tax_Amount;
         this.saleQuotationDetails[i].tax_Rate=0;
         this.saleQuotationDetails[i].tax_Amount=0;
     }
   }
    $("#TaxRateDropDown").prop("disabled", true); 
 }
