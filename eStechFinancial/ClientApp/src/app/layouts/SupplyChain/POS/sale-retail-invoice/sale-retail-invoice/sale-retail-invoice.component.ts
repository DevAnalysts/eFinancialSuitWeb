...
import { SaleOrderService, LoginService,SmsAlertService, SearchFilterService, saleOrder, Customers, CustomerService, saleOrderDetails, itemStockIMEI, EmailAlertService, PermissionUtility, SMSMessage, sendWhatsappMessage } from '../../../../../shared';
import swal from 'sweetalert';


@Component({
  selector: 'sale-retail-invoice',
  templateUrl: './sale-retail-invoice.component.html',
  styleUrls: ['./sale-retail-invoice.component.scss'],
  host: { '(window:keydown)': 'hotkeys($event)' }
})
export class SaleRetailInvoiceComponent implements OnInit, AfterViewChecked {
...
ImeiNumberValue(event:KeyboardEvent){
      ...
      .subscribe(response => {
        ...
        this.service.getContacts(this.customer_ID)
          .subscribe(response => {
            ...
            if(this.contacts[0].credit_Limit=='')
            this.credit_Limit=0;
            if(this.contacts[0].netPayable!=null)
            ...
            if (sessionStorage.getItem('EnableCustomerDiscount') != "1")
              if (sessionStorage.getItem('EnableDiscountGroup') != "0")
                if(this.contacts[0].discountRate>0)
        ...
        if (arr1 != null) {
          if (arr1.length > 0) {
            ...
            if (this.imei.length > 0) {
              this.getImeiItem(this.item_Code,1);
              $("#imei").focus().val('');
            }
            setTimeout(()=>{
            this.addGrid(this.item_Code,this.item_Name,this.unit_Price,this.Quantity,this.Discount_Rate,this.Tax_Rate,imei)
            },1000)
          }
          else {
            this.imei = '';
            swal("Serial# / IMEI# does not exist in database").then(r => {
              $("#imei").focus().val('');
            });
          }
        } else {
  ...
  getImeiItem(item_Code: any,sen:any) {
      ...
      this.service.getUnitPrice(this.item_Code, 0, false, this.userCurrentOffice, this.userCurrentWarehouse, this.customer_ID, this.unitID)
        .subscribe(response => {
          ...
          if (response.json() != null) {
            ...
            if(this.unitPrices[0].discountRate>0)
            ...
            this.service.getContacts(this.customer_ID)
              .subscribe(response => {
                ...
                if (response.json() != null) {
                  if (sessionStorage.getItem('EnableCustomerDiscount') != "1")
                    if (sessionStorage.getItem('EnableDiscountGroup') != "0")
                    alert(this.unitPrices[0].discountRate +'==='+ cont[0].discountRate);
                    if(cont[0].discountRate>0)
  ...
  changeItems(e: any, sender: any) {
    if (this.sessionEnableTextboxItem != 1) {
      if (sender == 0) {
        ...
        this.getImeiItem(this.item_Code,0);
        this.sender = 0;
      }
    }
  }
  ...
  addGrid(item_Code: any, item_Name: any, Unit_Price: any, Quantity: any, Discount_Rate: any, Tax_Rate: any, imei: any) {
    ...
    if(imei.length!=this.imeiLength && (this.category_Code==1 || this.category_Code==2))
    {
      swal("Length must be equal to max length", "", "info");
      return;
    }
  ...
  saveOrder(sale_Order_ID: any, sO_Date: any, office_Code: any, order_Envoy: any, customer_ID: any, contact_ID: any,
    ...
    if (flag) {
      ...
      // alert(this.totalNetPayable + '....' + (parseFloat(this.netPayable) + parseFloat(this.totalAmount) - (parseFloat(this.paid_Amount) - parseFloat(this.balance))));

      if (this.totalNetPayable < this.credit_Limit || this.totalAmount==this.cash) {
                  ...
                  this.service.saveOrder(order).then(
                    (response) => {
                     ...
                     console.log(this.idd);
                      ...
                      this.sendMessage(this.idd,customer_ID,total_Amount);
  ...
  getDetailsByID(sale_Order_ID, content) {
    ...
    this.service.getDetailsByID(sale_Order_ID)
      .subscribe((o: saleOrder) => {
        ...
        this.getPriviledgedOffices();
        this.service.getCustomersForSO(this.customer_ID)
          .subscribe(response => {
            this.customers = this.getDropdownList(response.json(), "customer_ID", "customer_Name");
          });
                        ...
                        this.service.getCustomerNo(customer_ID)
                          .subscribe(response => {
                            ...
                            let CustomerCell = list[0].cell;
                             ...
                             sendWhatsappMessage(CustomerCell, 'Mr./Mrs. ' + CustomerName + ' Your Invoice # ' + this.idd + ' of Rs. ' + this.totalAmount + ' has been issued.');
                          }
                          );
