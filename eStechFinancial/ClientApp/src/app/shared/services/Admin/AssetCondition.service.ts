...
import { AssetCondition } from '../../../shared';


@Injectable()
export class AssetConditionService {

  URL = 'api/AssetCondition';
   
  constructor(private httpRepository: RepositoryHttpService) { }

  //getAssetCategory
  getAssetCondition() {
    return this.httpRepository.getData(this.URL + "/getAssetCondition")
  }

  ////getDetailsByID
  getDetailsByID(ConditionCode: any) {
    return this.httpRepository.getData(this.URL + "/getDetailsByID?ConditionCode=" + ConditionCode)
  }

    //saveSubject
  saveAssetCondition(conditionasset: any): Promise<any> {
    let body = JSON.stringify(conditionasset);
  //  alert(body);
    return this.httpRepository.create(this.URL, conditionasset )
      .toPromise()
      .then(res => res.json() as AssetCondition)
      .catch()
  }

  ////////updateSubject
  UpdateAssetCondition(conditionasset: any): Promise<any> {
    let body = JSON.stringify(conditionasset);
    //  alert(body);
    return this.httpRepository.update(this.URL, conditionasset )
      .toPromise()
      .then(res => res.json() as AssetCondition)
      .catch()
  }
