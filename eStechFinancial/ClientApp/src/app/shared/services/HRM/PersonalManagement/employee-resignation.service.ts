...
import { EmployeeResignation } from '../../../../shared';

@Injectable()
export class EmployeeResignationService {

  URL = 'api/EmployeeResignation';
  ...
  //getDetailsByID
  getDetailsByID(Resignation_Code: any) {
    return this.httpRepository.getData(this.URL + "/getDetailsByID?Resignation_Code=" + Resignation_Code)
  }
  //searchEmpDetails
  ...
  saveEmployeeResignation(emppq: any): Promise<any> {
    let body = JSON.stringify(emppq);
     return this.httpRepository.create(this.URL, emppq )
      .toPromise()
      .then(res => res.json() as EmployeeResignation)
      .catch()
  }
  //updateEmployeeResignation
  updateEmployeeResignation(emppq: any): Promise<any> {
    let body = JSON.stringify(emppq);
